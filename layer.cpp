...
Layer::Layer(size_t in_dim, size_t out_dim, Activation act_func)
    : input_dim(in_dim), output_dim(out_dim), act(act_func),
    w(nullptr), b(nullptr), d_w(nullptr), d_b(nullptr) {
}
...
void Layer::init_weights(real_t* w_init, real_t* b_init) {
    if (w == nullptr) w = new real_t[input_dim * output_dim];
    if (b == nullptr) b = new real_t[output_dim];
...
void Layer::alloc_device() {
    ...
    if (w == nullptr || b == nullptr) {
        throw std::runtime_error("Host arrays not allocated. Call init_weights() first.");
    }
    ...
    err = cudaMemcpy(d_w, w, input_dim * output_dim * sizeof(real_t), cudaMemcpyHostToDevice);
    if (err != cudaSuccess) {
        cudaFree(d_w);
