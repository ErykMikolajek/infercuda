...
DatasetLoader::DatasetLoader(const std::string &path, size_t size,
                             size_t data_dim, size_t target_dim,
                             bool ignore_file_header)
    : dataset_path(path), dataset_size(size), current_index(0),
      data_dimension(data_dim), target_dimension(target_dim),
      ignore_header(ignore_file_header) {
...
DatasetLoader::~DatasetLoader() { dataset_file.close(); }

size_t DatasetLoader::get_dataset_size() const { return dataset_size; }
...
void DatasetLoader::allocate_on_device(real_t *data, real_t **allocated_data,
                                       size_t size) {
    cudaError_t err;
    ...
    err = cudaMemcpy(*allocated_data, data, size * sizeof(real_t),
...
real_t *DatasetLoader::deallocate_from_device(real_t **allocated_data,
                                              size_t data_size) {
    cudaError_t err;
